...
def adams_bohart_model(x, K, N, m, Vo, Co):
    return (np.exp((K * Co * x) - (K * m * N)/ Vo))

def wrapper_function(m, Vo, Co):
    def temp_func(x, K, q_o, m = m, Vo = Vo, Co = Co):
        return adams_bohart_model(x, K, q_o, m, Vo, Co)
    return temp_func

# perform the fit
def fit_adams_bohart_kinetic_model(data, m, Q, Co, name, value):
    model_params = {}
    ...
    y_pred = adams_bohart_model(xs, K, N, m, Vo, Co)
    data['q_calc'] = y_pred
    ...
    print(f"rsquared: {round(rSquared, 4)} Y = e^({round(K,6)} * {Co} * t) - ({K} * {m} * {round(N, 3)})/{Vo}")
